...
out of range are all refused — `null` is _not_ read as absence here, unlike `host`.
...
so a listener attached _after_ the synchronous `listen()` call still sees it — and, as in
