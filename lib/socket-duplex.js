...
const create = ({
  ...
  const currentLocalAddresses = socketCommon.getLocalAddresses({ native, fd });
  const currentRemoteAddresses = socketCommon.getRemoteAddresses({ native, fd });
  ...
  const currentMainLocalAddress = currentLocalAddresses[0];
  const currentMainRemoteAddress = currentRemoteAddresses[0];

  duplex.address = () => {
    return currentMainLocalAddress;
  };
  duplex.getLocalAddresses = () => {
    return currentLocalAddresses;
  };
  duplex.localFamily = currentMainLocalAddress.family;
  duplex.localAddress = currentMainLocalAddress.address;
  duplex.localPort = currentMainLocalAddress.port;

  duplex.getRemoteAddresses = () => {
    return currentRemoteAddresses;
  };
  duplex.remoteFamily = currentMainRemoteAddress.family;
  duplex.remoteAddress = currentMainRemoteAddress.address;
  duplex.remotePort = currentMainRemoteAddress.port;
