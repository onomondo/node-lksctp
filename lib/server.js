...
const listenArguments = ({ args }) => {
  ...
  // After the first argument, an optional one may be passed as undefined or
  // null in either form: net's normalizeArgs ignores anything it has no use
  // for, and callers forward optional values that way — listen(port, opts.host)
  // with no host configured, listen(options, opts.callback) with no callback.
  // The first slot is not filtered: it is the port or the options, a null
  // there is refused rather than read as absence, and dropping it would move a
  // host into the port's place.
  const [first, ...optional] = args;
  const rest = [first, ...optional.filter((arg) => arg !== undefined && arg !== null)];
  ...
  if (typeof rest[0] === "object" && rest[0] !== null) {
    ...
    // Only a value of the wrong kind is an error.
    options = { port: rest[0] };
...
const create = ({ native, options: socketOptions }) => {
  ...
  const raiseErrorAndClose = ({ error }) => {
    ...
    errored = true;
    // Deferred like 'listening', and like net's emitErrorNT: listen() calls
    // this synchronously on a failed bind, and a caller that attaches its
    // handler after listen() returns — `await events.once(server, "listening")`
    // — would otherwise miss the error and wait forever, while a caller with
    // no handler at all would see listen() throw it instead of the event.
  ...
  // The server is the emitter itself, as a net.Server is an EventEmitter: a
  // hand-picked on/once left out what callers use on one — events.once() calls
  // removeListener() when the event it waits for loses to 'error', and code
  // written against net calls off(), emit() and listenerCount() on its server.
  server = Object.assign(emitter, {
    address,
  ...
  Object.defineProperty(server, "listening", {
    get: () => listenPollHandle !== undefined,
    enumerable: true
  });
