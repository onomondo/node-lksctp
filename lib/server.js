...
const {
  ...
  getCurrentRemotePrimaryAddress: socketGetCurrentRemotePrimaryAddress
  ...
  const listen = (...args) => {
    ...
    listenPollHandle = pollerFactory.create({
      ...
      callback: () => {
        ...
        const { error: acceptError, fd: connfd } = accept({
          native,
          sockfd
        });
        ...
        const initialRemoteAddress = socketGetCurrentRemotePrimaryAddress({ native, fd: connfd });
