...
const sctp_bindx = ({ fd, sockaddrs, flags }) => {

  assert(typeof fd === "number");
  assert(Array.isArray(sockaddrs));
  sockaddrs.forEach((sockaddr) => {
    assert(sockaddr instanceof Uint8Array);
  });
  assert(typeof flags === "number");

  const { errno } = native.sctp_bindx({
    fd,
    sockaddrs,
    flags
  });
...
const sctp_connectx = ({ fd, sockaddrs }) => {
  ...
  const { errno } = native.sctp_connectx({
    fd,
    sockaddrs
  });
...
const getsockopt_peer_addr_info = ({ fd, sockaddr }) => {
...
const setsockopt_nodelay = ({ fd, value }) => {

  assert(typeof fd === "number");
  assert(typeof value === "number");

  const { errno } = native.setsockopt_nodelay({
    fd,
    value
  });
...
const setsockopt_sctp_event = ({ fd, se_type, se_on }) => {

  assert(typeof fd === "number");
  assert(typeof se_type === "number");
  assert(typeof se_on === "number");

  const { errno } = native.setsockopt_sctp_event({
    fd,
    se_type,
    se_on
  });
...
const sctp_getladdrs = ({ fd }) => {
  assert(typeof fd === "number");

  const { errno, sockaddrs } = native.sctp_getladdrs({
    fd
  });
  ...
  return { errno, sockaddrs };
};

const getpeername = ({ fd, sockaddr }) => {
  assert(typeof fd === "number");

  const { errno } = native.getpeername({
    fd,
    sockaddr
  });
...
const sctp_getpaddrs = ({ fd }) => {
  assert(typeof fd === "number");

  const { errno, sockaddrs } = native.sctp_getpaddrs({
    fd
  });
...
const parse_sctp_notification = ({ notification }) => {
  assert(Buffer.isBuffer(notification));

  const parsed = native.parse_sctp_notification({
    notification
  });

  assert(typeof parsed === "object");
  assert(typeof parsed.sn_type === "number");
