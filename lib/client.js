...
const bindLocalAddress = ({ native, sockfd, localAddress, localPort }) => {

  let localAddressToBind = "0.0.0.0";
  let localPortToBind = 0;

  if (localAddress !== undefined) {
    localAddressToBind = localAddress;
  }
  ...
  const family = determineAddressFamily({ address: localAddressToBind });

  const localSockaddr = sockaddrTranscoder.format({
    family,
    address: localAddressToBind,
    port: localPortToBind
  });
...
const connect = ({ native, options }) => {
  ...
  const { error: bindError } = bindLocalAddress({ native, sockfd, localAddress, localPort });
